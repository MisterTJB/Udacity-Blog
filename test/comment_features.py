...
def establish_users_and_post_with_comments():
    ...
    post_key = Post(title="Test", submitter="Test_User_01", content="Test").put()
...
class TestCommentFeatures(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_post_id, self.comment_ids = establish_users_and_post_with_comments()
    ...
    def testSignedInUserCanAddComment(self):
        ...
        latest_comment_content = latest_comment.select('.comment-content')[0].get_text()
    ...
    def testUserCanEditTheirOwnComment(self):
        ...
        post_page_response = self.testapp.request("/posts/%d" % self.test_post_id)
    ...
    def testUserCannotEditOtherComments(self):
        ...
        # Edit page is inaccessible for non-author (i.e. redirects to post page)
        edit_page_response = self.testapp.request(
            "/posts/%d/comments/%d" % (self.test_post_id, self.comment_ids[1]))
        redirect = edit_page_response.follow()
        self.assertEqual(redirect.request.path, '/posts/%d' % self.test_post_id)
    ...
    def testEditCommentViaPOSTIsIneffectiveForNonAuthor(self):
        ...
        url = "/posts/%d/comments/%d" % (self.test_post_id, self.comment_ids[1])
        ...
        edited_comment = second_comment.select('.comment-content')[0].get_text()
    ...
    def testUserCanDeleteTheirOwnComment(self):
        self.testapp.set_cookie('user', create_user_cookie('Test_User_01'))
        user_01_comment = "/posts/%d/comments/%d" % (self.test_post_id, self.comment_ids[0])
    ...
    def testUserCannotDeleteOtherComments(self):
        self.testapp.set_cookie('user', create_user_cookie('Test_User_01'))
        user_02_comment = "/posts/%d/comments/%d" % (
        self.test_post_id, self.comment_ids[1])
        delete_response = self.testapp.request(user_02_comment + "/delete")

        delete_redirect = delete_response.follow()
        self.assertEqual(delete_redirect.request.path, '/posts/%d' % self.test_post_id)
